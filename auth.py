...
def register(password):
    if os.path.isfile(f"{os.getenv('HOME')}/passmgr_data/password") is True:
        ...
        os.makedirs(f"{os.getenv('HOME')}/passmgr_data")
        password_hash = crypt.sha256_hash(password)
        file = open(f"{os.getenv('HOME')}/passmgr_data/password", "w")
...
def login(password):
    if os.path.isfile(f"{os.getenv('HOME')}/passmgr_data/password") is False:
        os.system("clear")
        print("[ERROR] Password file doesn't exist, quitting!")
        app.quit_app()
    else:
        password_hash = crypt.sha256_hash(password)
        file = open(f"{os.getenv('HOME')}/passmgr_data/password", "r")
        ...
        if password_hash == file_password_hash:
            ...
            if os.path.isfile(f"{os.getenv('HOME')}/passmgr_data/passwords.db") is False:
                database.create_db()
...
def verified():
    ...
    login_status = True
    with open(f"{os.getenv('HOME')}/passmgr_data/password", 'r') as file:
